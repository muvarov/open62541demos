...
static void read_time_cb(UA_Client *client, void *userdata,
			 UA_UInt32 requestId, UA_Variant *var) {
	printf ("%s() %-50s%i\n", "%s read value attribute for request", __func__, requestId);

	if (UA_Variant_hasScalarType(var, &UA_TYPES[UA_TYPES_DATETIME])) {
		UA_DateTime val = *(UA_DateTime *)var->data;

		printf("---%s %ld\n",
				"Reading the value of node (1, \"current time\"):", val);
	}
...
main (int argc, char *argv[]) {
	...
	for (UA_UInt16 i = 0; i < 5; i++) {
		...
		if (UA_Client_getState (client) == UA_CLIENTSTATE_SESSION) {
			...
			printf("call for current time read\n");
			UA_Client_readValueAttribute_async(client,
					UA_NODEID_STRING (1, "current.time"),
					read_time_cb, NULL,
					&reqId);
